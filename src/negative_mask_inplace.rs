...
unsafe extern "C" {
    fn lufloat_negative_mask_inplace(data: *mut u16, size: usize) -> c_int;
}
...
pub(crate) fn apply(buffer: &mut UnifiedBuffer<'_>) {
    ...
    while remaining > 0 {
        ...
        let err = unsafe { lufloat_negative_mask_inplace(ptr, current) };
...
mod tests {
    ...
    #[test]
    fn exhaustive_lufloat_negative_mask_inplace() {
