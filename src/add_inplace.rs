...
unsafe extern "C" {
    fn lufloat_add_inplace(data: *mut u16, other: *const u16, size: usize) -> c_int;
}
...
pub(crate) fn apply(data: &mut UnifiedBuffer<'_>, other: &UnifiedBuffer<'_>) {
    ...
    while remaining > 0 {
        ...
        let err = unsafe { lufloat_add_inplace(ptr, other_ptr, current) };
...
mod tests {
    ...
    #[test]
    fn exhaustive_lufloat_add_inplace() {
