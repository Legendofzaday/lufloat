...
unsafe extern "C" {
    fn lufloat_silu(data: *const u16, size: usize, activation: *mut u16) -> c_int;
}
...
pub(crate) fn apply<'a>(data: &UnifiedBuffer<'a>, activation: &mut UnifiedBuffer<'a>) {
    ...
    while remaining > 0 {
        ...
        let err = unsafe { lufloat_silu(data_ptr, current, activation_ptr) };
