...
unsafe extern "C" {
    fn lufloat_negative_mask(data: *const u16, size: usize, mask: *mut u16) -> c_int;
}
...
pub(crate) fn apply<'a>(data: &UnifiedBuffer<'a>, mask: &mut UnifiedBuffer<'a>) {
    ...
    while remaining > 0 {
        ...
        let err = unsafe { lufloat_negative_mask(data_ptr, current, mask_ptr) };
...
mod tests {
    ...
    #[test]
    fn exhaustive_lufloat_negative_mask() {
