...
unsafe extern "C" {
    fn lufloat_sigma(data: *const u16, size: usize, activation: *mut u16) -> c_int;
}
...
pub(crate) fn apply<'a>(data: &UnifiedBuffer<'a>, activation: &mut UnifiedBuffer<'a>) {
    ...
    while remaining > 0 {
        ...
        let err = unsafe { lufloat_sigma(data_ptr, current, activation_ptr) };
